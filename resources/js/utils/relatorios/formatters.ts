...
export const formatters = {
	...
	currency: (
		value: number,
		emptyText: string = "Sem valor informado",
	): string => {
	...
	date: (
		dateString: string,
		emptyText: string = "Data não informada",
	): string => {
		if (!dateString) {
			return emptyText;
		}
...
export const reportUtils = {
	...
	getEmptyStateMessage: (
		...
		const messages = {
			requisicoes: filterActive
				? "Nenhuma requisição encontrada com os filtros aplicados"
				: "Nenhuma requisição cadastrada no sistema",
			fornecedores: filterActive
				? "Nenhum fornecedor encontrado com os filtros aplicados"
				: "Nenhum fornecedor cadastrado no sistema",
			conferencias: filterActive
				? "Nenhuma conferência encontrada com os filtros aplicados"
				: "Nenhuma conferência realizada no período",
		};
	...
	getTopEntries: <T>(
		...
		return reportUtils
			.sortByValue(Object.entries(data), getValue)
			.slice(0, limit);
	},
...
export const cardMetrics = {
	...
	createMetricCard: (
		...
		value:
			formatter === "currency"
				? formatters.currency(value, emptyText)
				: formatters.number(value, emptyText),
		subtitle,
		isEmpty: value === 0,
	}),
};
