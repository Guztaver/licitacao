...
import {
	EMITENTE_LABELS,
	EMITENTE_MESSAGES,
	EMITENTE_STATUS_CONFIG,
	EMITENTE_TABLE_COLUMNS,
	type EmitenteStatus,
...
export function EmitenteEmptyState({
	isFiltered,
	onClearFilters,
	onSuccess,
}: EmitenteEmptyStateProps) {
	return (
		<TableRow>
		...
		<TableRow>
			<TableCell>
				<div>
					...
					{emitente.endereco && (
						<p className="text-sm text-gray-500">{emitente.endereco}</p>
					)}
				</div>
			</TableCell>
			<TableCell>
			...
			<TableCell>
				{formatRequisicoesCount(emitente.requisicoes_count || 0)}
			</TableCell>
			<TableCell>
				...
				<TableBody>
					{hasResults ? (
						emitentes.map((emitente) => (
							<EmitenteTableRow key={emitente.id} emitente={emitente} />
						))
					) : (
						<EmitenteEmptyState
...
export function EmitenteRequisicoesTable({
	requisicoes,
	totalCount,
	emitenteId,
}: EmitenteRequisicoesTableProps) {
	if (!requisicoes || requisicoes.length === 0) {
		return (
			<div className="py-8 text-center">
				<FileText className="mx-auto h-12 w-12 text-gray-400" />
				<h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">
					{EMITENTE_MESSAGES.noRequisicoes}
				</h3>
				<p className="mt-1 text-sm text-gray-500">
					{EMITENTE_MESSAGES.noRequisicoesDescription}
				</p>
			</div>
		);
	}
...
export function EmitenteListCard({
	title,
	description,
	children,
	filterSummary,
	pagination,
}: EmitenteListCardProps) {
	return (
		<Card>
			<CardHeader>
