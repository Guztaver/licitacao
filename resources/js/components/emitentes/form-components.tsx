...
import {
	EMITENTE_FORM_FIELDS,
	EMITENTE_LABELS,
	EMITENTE_MESSAGES,
	type EmitenteFormField,
...
export function EmitenteInputField({
	field,
	value,
	onChange,
	error,
...
export function EmitenteBasicInfoSection({
	data,
	onChange,
	errors,
	disabled = false,
}: EmitenteBasicInfoSectionProps) {
	const basicFields = EMITENTE_FORM_FIELDS.filter((field) =>
...
export function EmitenteAddressSection({
	data,
	onChange,
	errors,
	disabled = false,
}: EmitenteAddressSectionProps) {
	const addressField = EMITENTE_FORM_FIELDS.find(
		(field) => field.name === "endereco",
	);
...
export function EmitenteContactSection({
	data,
	onChange,
	errors,
	disabled = false,
}: EmitenteContactSectionProps) {
	const contactFields = EMITENTE_FORM_FIELDS.filter((field) =>
...
export function EmitenteObservationsSection({
	value,
	onChange,
	error,
	disabled = false,
}: EmitenteObservationsSectionProps) {
...
export function EmitenteFormCard({
	data,
	onChange,
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					{isEdit
						? EMITENTE_MESSAGES.editFormDescription
						: EMITENTE_MESSAGES.formDescription}
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				{/* Basic Info */}
				<EmitenteBasicInfoSection
					data={data}
					onChange={onChange}
					errors={errors}
					disabled={disabled}
	...
	return (
		<div className="flex items-center justify-end space-x-4">
			...
			<Button type="submit" disabled={processing} onClick={onSubmit}>
				...
				{processing
					? EMITENTE_LABELS.saving
					: isEdit
						? EMITENTE_LABELS.saveChanges
						: EMITENTE_LABELS.save}
			</Button>
		</div>
	);
}
