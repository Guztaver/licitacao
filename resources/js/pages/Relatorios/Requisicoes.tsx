...
export default function RelatorioRequisicoes({ requisicoes, stats, filters, emitentes = [], fornecedores = [] }: RelatorioRequisicoesProps) {
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            ...
            autorizada: { label: 'Autorizada', className: 'bg-blue-100 text-blue-800 border-blue-200' },
            concretizada: { label: 'Concretizada', className: 'bg-green-100 text-green-800 border-green-200' },
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="grid gap-4 md:grid-cols-4">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-green-600">{formatCurrency(stats.valor_total)}</div>
                            <p className="text-xs text-muted-foreground">requisições concretizadas</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-amber-600">{stats.requisicoes_por_status.pendente || 0}</div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Use os filtros para refinar o relatório</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{stats.total_requisicoes} requisições encontradas</CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    {requisicoes.length > 0 ? (
                                            ...
                                            <TableRow key={requisicao.id}>
                                                ...
                                                <TableCell>
                                                    {requisicao.fornecedor ? (
                                                        <div className="font-medium">{requisicao.fornecedor.razao_social}</div>
                                                    ) : (
                                                        <span className="text-gray-400">-</span>
                                                    )}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {requisicao.valor_final ? (
                                                        <span className="font-medium">{formatCurrency(requisicao.valor_final)}</span>
                                                    ) : (
                                                        <span className="text-gray-400">-</span>
                                                    )}
                                                </TableCell>
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={7} className="py-8 text-center">
                                                ...
                                                <p className="mt-2 text-gray-500">Nenhuma requisição encontrada com os filtros aplicados</p>
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    {Object.entries(stats.valor_por_emitente)
                                        ...
                                        .slice(0, 5)
                                        .map(([emitente, valor]) => (
                                            <div key={emitente} className="flex items-center justify-between">
                                                <span className="text-sm font-medium">{emitente}</span>
                                                <span className="text-sm text-green-600">{formatCurrency(Number(valor))}</span>
                                            </div>
                                        ))}
                                </div>
                            </CardContent>
