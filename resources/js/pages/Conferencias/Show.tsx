...
const FORNECEDOR_FIELDS = [
    {
        ...
        label: 'Razão Social',
        format: (value: string | number | null | undefined) => value?.toString() || MESSAGES.noValue,
        isBold: true,
    },
    {
...
function InfoField({
    label,
    value,
    format = (v: string | number | null | undefined) => v?.toString() || MESSAGES.noValue,
    isBold = false,
    isMonospace = false,
}: InfoFieldProps) {
    ...
    const valueClasses = `text-sm ${isBold ? 'font-semibold' : ''} ${isMonospace ? 'font-mono' : ''}`;
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-3">
                    {SUMMARY_CONFIG.map(({ key, title, color, textColor, valueColor }) => (
                        <div key={key} className={`rounded-lg p-4 ${color}`}>
                            <p className={`text-sm font-medium ${textColor}`}>{title}</p>
                            <p className={`text-2xl font-bold ${valueColor}`}>{formatCurrency(conferencia[key])}</p>
                        </div>
                    ))}
                </div>
            </CardContent>
    ...
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <div>
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{periodFields[2].label}</p>
...
function ObservationsCard({ observacoes }: ObservationsCardProps) {
    return (
        <Card>
            ...
            <CardContent>
                <p className="whitespace-pre-wrap text-gray-700 dark:text-gray-300">{observacoes}</p>
            </CardContent>
        </Card>
    );
}
...
function FornecedorCard({ fornecedor }: FornecedorCardProps) {
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                {FORNECEDOR_FIELDS.map(({ key, label, format, isBold, isMonospace }) => (
                    <InfoField key={key} label={label} value={fornecedor[key]} format={format} isBold={isBold} isMonospace={isMonospace} />
                ))}
        ...
        <Card>
            ...
            <CardContent className="space-y-3">
                {SYSTEM_FIELDS.map(({ key, label, format, isMonospace }) => (
                    <InfoField key={key} label={label} value={conferencia[key]} format={format} isMonospace={isMonospace} />
                ))}
            </CardContent>
        </Card>
    );
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        {/* Observations */}
                        {conferencia.observacoes && <ObservationsCard observacoes={conferencia.observacoes} />}
                    </div>
