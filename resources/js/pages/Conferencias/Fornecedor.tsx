		...
		<Card>
			...
			<CardContent>
				<div className="grid gap-4 md:grid-cols-3">
					{TOTALS_CONFIG.map(
						({ key, title, color, textColor, valueColor, countLabel }) => {
							const count =
								key === "requisicoes"
									? requisicoes.length
									: key === "pedidos_manuais"
										? pedidos_manuais.length
										: 0;
			...
			<TableCell>
				{!isEditing && (
					<Button
						size="sm"
						variant="outline"
						onClick={() => onEdit(requisicao.id, requisicao.valor_final)}
					>
...
function PedidosManuaisTable({
	pedidos_manuais,
	onDelete,
}: PedidosManuaisTableProps) {
	if (pedidos_manuais.length === 0) {
		return (
			<div className="rounded-lg bg-gray-50 p-8 text-center dark:bg-gray-900">
				<Plus className="mx-auto h-12 w-12 text-gray-400" />
				<h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">
					{MESSAGES.noPedidosManuais}
				</h3>
				<p className="mt-2 text-gray-600 dark:text-gray-400">
					{MESSAGES.noPedidosManuaisDescription}
				</p>
			</div>
		);
	}
				...
				<div className="grid gap-6 lg:grid-cols-4">
					...
					<div className="space-y-6 lg:col-span-3">
						...
						<FinancialSummary
							...
							pedidos_manuais={pedidos_manuais}
						/>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								...
								{!showAddPedidoForm && (
									<PedidosManuaisTable
										pedidos_manuais={pedidos_manuais}
										onDelete={handleDeletePedidoManual}
									/>
								)}
							</CardContent>
						</Card>
