...
export default function ConferenciaFornecedor({
    ...
    const handleFinalizarConferencia = () => {
        if (confirm('Tem certeza que deseja finalizar esta conferência? Esta ação não pode ser desfeita.')) {
                    ...
                    <div className="space-y-6 lg:col-span-3">
                            ...
                            <CardContent>
                                <div className="grid gap-4 md:grid-cols-3">
                                    <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
                                        ...
                                        <p className="text-2xl font-bold text-blue-900 dark:text-blue-100">{formatCurrency(totals.requisicoes)}</p>
                                        <p className="text-xs text-blue-600 dark:text-blue-400">{requisicoes.length} requisições</p>
                                    </div>
                                    <div className="rounded-lg bg-green-50 p-4 dark:bg-green-900/20">
                                        ...
                                        <p className="text-xs text-green-600 dark:text-green-400">{pedidos_manuais.length} pedidos</p>
                                    </div>
                                    <div className="rounded-lg bg-purple-50 p-4 dark:bg-purple-900/20">
                                        <p className="text-sm font-medium text-purple-600 dark:text-purple-400">Total Geral</p>
                                        <p className="text-2xl font-bold text-purple-900 dark:text-purple-100">{formatCurrency(totals.geral)}</p>
                                        <p className="text-xs text-purple-600 dark:text-purple-400">Valor final da conferência</p>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                            ...
                            <CardContent>
                                {requisicoes.length > 0 ? (
                                            ...
                                            <TableBody>
                                                {requisicoes.map((requisicao) => (
                                                    <TableRow key={requisicao.id}>
                                                        ...
                                                        <TableCell>
                                                            {requisicao.emitente ? `${requisicao.emitente.nome} (${requisicao.emitente.sigla})` : '-'}
                                                        </TableCell>
                                                        <TableCell>
                                                            {editingRequisicao === requisicao.id ? (
                                                                <div className="flex items-center space-x-2">
                                                                    <Input
                                                                        ...
                                                                        onChange={(e) => setRequisicaoData('valor_final', e.target.value)}
                                                                        className="w-24"
                                                                    />
                                                                    <Button
                                                                        size="sm"
                                                                        onClick={() => handleSaveRequisicao(requisicao.id)}
                                                                        disabled={processingRequisicao}
                                                                    >
                                                                        <Save className="h-3 w-3" />
                                                                    </Button>
                                                                    <Button size="sm" variant="outline" onClick={() => setEditingRequisicao(null)}>
                                                                        Cancelar
                                                                    </Button>
                                                                </div>
                                                            ) : (
                                                        ...
                                                        <TableCell>
                                                            {editingRequisicao !== requisicao.id && (
                                                                <Button
                                                                    size="sm"
                                                                    variant="outline"
                                                                    onClick={() => handleEditRequisicao(requisicao.id, requisicao.valor_final)}
                                                                >
                                    ...
                                    <div className="rounded-lg bg-gray-50 p-8 text-center dark:bg-gray-900">
                                        ...
                                        <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">Nenhuma requisição encontrada</h3>
                                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                                            Não há requisições concretizadas para este fornecedor no período selecionado.
                                        </p>
                                    </div>
                                )}
                            </CardContent>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                {/* Add Pedido Form */}
                                {showAddPedidoForm && (
                                    <form onSubmit={handleAddPedidoManual} className="space-y-4 rounded-lg border p-4">
                                        ...
                                        <div className="grid gap-4 md:grid-cols-2">
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setPedidoData('descricao', e.target.value)}
                                                    placeholder="Descreva o pedido..."
                                                    required
                                                    className={pedidoErrors.descricao ? 'border-red-500' : ''}
                                                />
                                                {pedidoErrors.descricao && <p className="text-sm text-red-500">{pedidoErrors.descricao}</p>}
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setPedidoData('valor', e.target.value)}
                                                    placeholder="0,00"
                                                    required
                                                    className={pedidoErrors.valor ? 'border-red-500' : ''}
                                                />
                                                {pedidoErrors.valor && <p className="text-sm text-red-500">{pedidoErrors.valor}</p>}
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setPedidoData('numero_pedido', e.target.value)}
                                                    placeholder="Ex: PED-001"
                                                    className={pedidoErrors.numero_pedido ? 'border-red-500' : ''}
                                                />
                                                {pedidoErrors.numero_pedido && <p className="text-sm text-red-500">{pedidoErrors.numero_pedido}</p>}
                                            </div>
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setPedidoData('data_pedido', e.target.value)}
                                                    required
                                                    className={pedidoErrors.data_pedido ? 'border-red-500' : ''}
                                                />
                                                {pedidoErrors.data_pedido && <p className="text-sm text-red-500">{pedidoErrors.data_pedido}</p>}
                                            </div>
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Textarea
                                                ...
                                                onChange={(e) => setPedidoData('observacoes', e.target.value)}
                                                placeholder="Justifique a necessidade deste pedido manual..."
                                                required
                                                rows={3}
                                                className={pedidoErrors.observacoes ? 'border-red-500' : ''}
                                            />
                                            {pedidoErrors.observacoes && <p className="text-sm text-red-500">{pedidoErrors.observacoes}</p>}
                                        </div>
                                        <div className="flex items-center space-x-2">
                                ...
                                {pedidos_manuais.length > 0 ? (
                                                    ...
                                                    <TableRow key={pedido.id}>
                                                        ...
                                                        <TableCell>
                                                            <Button
                                                                size="sm"
                                                                variant="destructive"
                                                                onClick={() => handleDeletePedidoManual(pedido.id)}
                                                            >
                                    ...
                                    !showAddPedidoForm && (
                                        <div className="rounded-lg bg-gray-50 p-8 text-center dark:bg-gray-900">
                                            ...
                                            <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">Nenhum pedido manual</h3>
                                            <p className="mt-2 text-gray-600 dark:text-gray-400">
                                                Clique em "Adicionar Pedido" para incluir pedidos manuais nesta conferência.
                                            </p>
                                        </div>
                                    )
                                )}
                        ...
                        <Card className="bg-blue-50 dark:bg-blue-950/20">
                            <CardContent className="pt-6">
                                <div className="flex">
                                    ...
                                    <div className="ml-3">
                                        <h3 className="text-sm font-medium text-blue-800 dark:text-blue-200">Instruções</h3>
                                        <div className="mt-2 text-sm text-blue-700 dark:text-blue-300">
                                            <p>
                                                • Revise as requisições e ajuste valores se necessário
                                                <br />• Adicione pedidos manuais com justificativa obrigatória
                                                <br />• Clique em "Finalizar Conferência" quando terminar
                                            </p>
                                        </div>
                                    </div>
                                </div>
