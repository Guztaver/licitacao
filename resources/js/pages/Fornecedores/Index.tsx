...
export default function FornecedoresIndex({
	...
	stats,
	filters,
}: FornecedoresIndexProps) {
	// Add safety checks for data
	const safeFornecedores = fornecedoresPaginated || {
		data: [],
		links: [],
	...
	const safeMeta = safeFornecedores.meta || {
		total: 0,
		from: 0,
		to: 0,
		last_page: 1,
	...
	return (
		<AppLayout breadcrumbs={breadcrumbs}>
			...
			<div className="flex h-full flex-1 flex-col gap-6 p-6">
				...
				<div className="grid gap-4 md:grid-cols-4">
					<Card>
						...
						<CardContent>
							<div className="text-2xl font-bold">
								{safeStats.total_fornecedores}
							</div>
							<p className="text-xs text-muted-foreground">
								{data.search || data.status
									? "encontrados com filtros"
									: "fornecedores cadastrados"}
							</p>
							{safeData.length > 0 &&
								safeMeta.total !== safeStats.total_fornecedores && (
					...
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">
								{data.search || data.status
									? "com requisições nos resultados"
									: "possuem requisições/pedidos"}
							</p>
						</CardContent>
					</Card>
						...
						<div className="flex">
							<div className="ml-3">
								...
								<p className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
									Não foram encontrados fornecedores que correspondam aos
				...
				{/* Low Data Notice */}
				{safeStats.total_fornecedores > 0 &&
					safeStats.com_requisicoes === 0 &&
					!data.search &&
					!data.status && (
							...
							<div className="flex">
								<div className="ml-3">
									...
									<p className="mt-2 text-sm text-blue-700 dark:text-blue-300">
										Existem fornecedores cadastrados, mas nenhum possui
										requisições ou pedidos manuais associados. Para gerar
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Mostrando {fornecedoresPaginated.data.length} de{" "}
							{fornecedoresPaginated.meta?.total || 0} fornecedores
						</CardDescription>
					</CardHeader>
					<CardContent>
											...
											<TableRow key={fornecedor.id}>
												...
												<TableCell>
													<div className="text-sm">
														{fornecedor.telefone_formatado && (
															<p>{fornecedor.telefone_formatado}</p>
														)}
														{fornecedor.email && (
												...
												<TableCell>
													<div className="text-sm">
														...
														{fornecedor.conferencias_count &&
															fornecedor.conferencias_count > 0 && (
																<p className="text-gray-500">
																	{fornecedor.conferencias_count} conferências
																</p>
															)}
													</div>
												</TableCell>
												<TableCell>
													{fornecedor.total_geral
														? formatCurrency(fornecedor.total_geral)
														: "-"}
												</TableCell>
												<TableCell>
													<div className="flex items-center space-x-2">
						...
						{fornecedoresPaginated.meta?.last_page > 1 && (
							<div className="flex items-center justify-between px-2 py-4">
								<div className="text-sm text-gray-700">
									Mostrando {fornecedoresPaginated.meta?.from || 0} até{" "}
									{fornecedoresPaginated.meta?.to || 0} de{" "}
