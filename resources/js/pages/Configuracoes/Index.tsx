...
export default function ConfiguracoesIndex({ configuracoes }: ConfiguracoesIndexProps) {
    const { data, setData, post, processing, errors } = useForm<ConfiguracoesData>(getFormDefaults(configuracoes));
    ...
    const renderHeader = useCallback(
        () => (
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{CONFIGURACOES_MESSAGES.PAGE_TITLE}</h1>
                    <p className="text-gray-600 dark:text-gray-400">{CONFIGURACOES_MESSAGES.PAGE_DESCRIPTION}</p>
                </div>
            </div>
        ),
        [],
        ...
        () => (
            <div className="flex justify-end space-x-4">
                <Button type="submit" disabled={processing}>
                    ...
                    {processing ? CONFIGURACOES_MESSAGES.SAVING : CONFIGURACOES_MESSAGES.SAVE_BUTTON}
                </Button>
            </div>
        ),
        [processing],
