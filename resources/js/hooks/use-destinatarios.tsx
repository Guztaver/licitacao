...
export function useDestinatarisForm({ initialData, destinatarioId, onSuccess, onError }: UseDestinatarisFormOptions = {}) {
    const isEditing = Boolean(destinatarioId);

    const { data, setData, post, put, processing, errors, reset } = useForm<DestinatarisFormData>({
        ...DEFAULT_FORM_DATA,
        ...initialData,
    });
...
export function useDestinatarisFilters({ initialFilters }: UseDestinatarisFiltersOptions = {}) {
...
export function useDestinatarisActions({ onSuccess, onError }: UseDestinatarisActionsOptions = {}) {
    const deleteDestinatario = useCallback(
        (id: number, name: string) => {
            const confirmed = window.confirm(`Tem certeza que deseja excluir o destinatário "${name}"?\n\nEsta ação não pode ser desfeita.`);
    ...
    const exportDestinatarios = useCallback((filters?: { search?: string }) => {
        ...
        const url = params.toString() ? `${destinatarios.export()}?${params.toString()}` : destinatarios.export();
