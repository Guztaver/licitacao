 ...
 */
export function useEmitenteForm(initialData?: Partial<EmitenteFormData>, emitenteId?: number) {
    ...
    const { data, setData, post, put, processing, errors, clearErrors } = useForm<EmitenteFormData>(defaultData);
 ...
 */
export function useEmitenteBreadcrumbs(page: 'index' | 'create' | 'edit' | 'show', emitente?: Emitente): BreadcrumbItem[] {
    return useMemo(() => {
...
export function useEmitenteActions() {
    const handleDelete = useMemo(
        () => (emitente: Emitente) => {
            if (confirm(`${EMITENTE_LABELS.deleteConfirmMessage}\n\nEmitente: ${emitente.nome}`)) {
        ...
        () => (field: keyof EmitenteFormData, value: string) => {
            switch (field) {
                ...
                case 'sigla':
                    ...
                    if (value.length > 10) return 'Sigla deve ter no máximo 10 caracteres';
                    if (!/^[A-Z0-9]+$/.test(value)) return 'Sigla deve conter apenas letras maiúsculas e números';
                    break;
                case 'email':
                    if (value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return 'Formato de email inválido';
                    break;
                case 'telefone':
                    if (value && !/^(\(?\d{2}\)?\s?)?\d{4,5}-?\d{4}$/.test(value)) return 'Formato de telefone inválido';
                    break;
                case 'endereco':
                    if (value.length > 500) return 'Endereço deve ter no máximo 500 caracteres';
                    break;
                case 'observacoes':
                    if (value.length > 1000) return 'Observações devem ter no máximo 1000 caracteres';
                    break;
            }
            return null;
        },
