...
class RelatorioController extends Controller
{
    ...
    public function requisicoes(Request $request): Response
    {
        ...
        $stats = [
            ...
            'requisicoes_por_status' => $requisicoes->groupBy('status')->map(function ($group) {
                return $group->count();
            }),
        ...
        $requisicoesList = $requisicoes->map(function ($requisicao) {
            return [
                'id' => $requisicao->id,
    ...
    public function fornecedores(Request $request): Response
    {
        ...
        $fornecedoresList = $fornecedores->map(function ($fornecedor) use ($request) {
            $requisicoesQuery = $fornecedor->requisicoes()->concretizada();
    ...
    public function conferencias(Request $request): Response
    {
        ...
        $stats = [
            ...
            'conferencias_por_fornecedor' => $conferencias->groupBy('fornecedor.razao_social')->map(function ($group) {
                return [
                    'count' => $group->count(),
                    'valor_total' => $group->sum('total_geral'),
                ];
            }),
            'conferencias_por_mes' => $conferencias->groupBy(function ($conferencia) {
                return \Carbon\Carbon::parse($conferencia->data_conferencia)->format('Y-m');
            })->map(function ($group) {
                return [
                    'count' => $group->count(),
                    'valor_total' => $group->sum('total_geral'),
        ...
        $conferenciasList = $conferencias->map(function ($conferencia) {
            return [
                'id' => $conferencia->id,
