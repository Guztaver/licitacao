...
class ItemController extends Controller
{
    ...
    public function destroy(Item $item): RedirectResponse
    {
        ...
        if ($item->foiUtilizado()) {
            return redirect()
                ...
                ->with(
                    "error",
                    "Este item não pode ser excluído porque já foi utilizado em transações.",
                );
        }
    ...
    /**
     * Show audit logs for the item.
     */
    public function auditLogs(Item $item): Response
    {
        $logs = $item->auditLogs()->with("user")->paginate(50);

        return Inertia::render("Items/AuditLogs", [
            "item" => [
                "id" => $item->id,
                "codigo" => $item->codigo,
                "descricao" => $item->descricao,
                "is_frozen" => $item->is_frozen,
                "frozen_at" => $item->frozen_at?->format("d/m/Y H:i"),
                "frozen_by_user" => $item->frozen_by_user_id
                    ? User::find($item->frozen_by_user_id)?->name
                    : null,
                "usage_stats" => $item->getEstatisticasUso(),
            ],
            "logs" => $logs->getCollection()->map(function ($log) {
                return [
                    ...
                    "field_display" => $log->field_display,
    ...
    /**
     * Force freeze an item (admin action).
     */
    public function freeze(Request $request, Item $item): RedirectResponse
    {
        $this->authorize("manage", Item::class);

        try {
            $item->congelar(auth()->id());

            return redirect()
                ->back()
                ->with("success", "Item congelado com sucesso!");
        } catch (\Exception $e) {
            return redirect()
                ->back()
                ->with("error", "Erro ao congelar item: " . $e->getMessage());
        }
    }
